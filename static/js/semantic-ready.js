...
semantic.ready = function() {
  ...
  var
    ...
    $subWaypoints     = $peek.closest('.tab, .container').find('h3').first().siblings('h3').addBack(),
          ...
          .promise()
            .done(function() {
              ...
              $headers
                .removeClass('active')
              ;
              $header.parent().find('.active').removeClass('active');
              $header
                .addClass('active')
              ;
              $waypoint
  ...
  */
  ...
  $subWaypoints
    .waypoint({
      ...
      handler    : function(direction) {
        var
          subIndex = (direction == 'down')
            ? $subWaypoints.index(this)
            : ($subWaypoints.index(this) - 1 >= 0)
              ? ($subWaypoints.index(this) - 1)
              : 0
        ;
        $peekSubItem
          .removeClass('active')
        ;
        if ($peekSubItem.eq(subIndex).parents('.item').hasClass('active')) {
          $peekSubItem
            .eq( subIndex )
            .addClass('active')
          ;
