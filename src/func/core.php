 ...
 *      SUSTC-ShenZhen-B
 *
 *      $Id: func/core.php 2013-06-13 04:20:00 Zhuoteng Peng $
 */
...
define('BM_CORE_FUNCTION', true);

function system_error($message, $show = true, $save = true, $halt = true) {
	bm_error::system_error($message, $show, $save, $halt);
}
...
function getuserbyuid($uid, $fetch_archive = 0, $get_avatar = 1) {
	static $users = array();
	if(empty($users[$uid])) {
		...
		if ($get_avatar && !empty($users[$uid])) {
			$users[$uid]['avatar_url'] = getuseravatar($uid, $users[$uid]);
		}
	}
...
function getuseravatar($uid, $user = array()) {
	if (!$user) {
		$user = getuserbyuid($uid, 0, 0);
	}
	if ($user) {
		if (!$user['avatarstatus']) {
			return '/static/img/user/def-avatar.png';
		} else {
			$avatar_file = '/data/attach/avatar/'.$uid.'.jpg';
			$time = @filectime(BM_ROOT.$avatar_file);
			if ($time) {
				$avatar_file .= '?' . date('YmdHis', $time);
			}
			return $avatar_file;
		}
	} else {
...
function hookscript($script, $hscript, $type = 'funcs', $param = array(), $func = '', $scriptextra = '') {
	...
	foreach((array)$_G['setting'][HOOKTYPE][$hscript][$script]['module'] as $identifier => $include) {
		...
		if($hooksadminid[$identifier]) {
			@include_once BM_ROOT.'./source/plugin/'.$include.'.class.php';
		}
	}
...
function output() {
	...
	if(defined('BM_OUTPUTED')) {
		return;
	} else {
		define('BM_OUTPUTED', 1);
	}
	...
	if(defined('CACHE_FILE') && CACHE_FILE && !defined('CACHE_FORBIDDEN') && !defined('IN_MOBILE') && !checkmobile()) {
		if(diskfreespace(BM_ROOT.'./'.$_G['setting']['cachethreaddir']) > 1000000) {
	...
	if(defined('BM_DEBUG') && BM_DEBUG && @include(libfile('function/debug'))) {
...
function lang($file, $langvar = null, $vars = array(), $default = null) {
	...
	if($path != 'plugin') {
		...
		if(!isset($_G['lang'][$key])) {
			include BM_ROOT.'./src/lang/'.($path == '' ? '' : $path.'/').$file.'.php';
			$_G['lang'][$key] = $lang;
		}
		if(defined('IN_MOBILE') && !defined('TPL_DEFAULT')) {
			include BM_ROOT.'./src/lang/mobile/'.$langname.'/template.php';
		...
		if(empty($_G['config']['plugindeveloper'])) {
			...
			if(@include(BM_ROOT.'./data/plugindata/'.$file.'.lang.php')) {
...
function checktplrefresh($maintpl, $subtpl, $timecompare, $templateid, $cachefile, $tpldir, $file) {
	...
	if(empty($timecompare) || $tplrefresh == 1 || ($tplrefresh > 1 && !($timestamp % $tplrefresh))) {
		if(empty($timecompare) || @filemtime(BM_ROOT.$subtpl) > $timecompare) {
			require_once BM_ROOT.'/src/class/template.php';
...
function template($file, $templateid = 0, $tpldir = '', $gettplfile = 0, $primaltpl='') {
	global $_G;
	require_once libfile('func/jade');
/*
	...
	if(strpos($file, ':') !== false) {
		...
		if($templateid == 'diy') {
			...
			$_G['style']['prefile'] = '';
			$diypath = BM_ROOT.'./data/diy/'.$_G['style']['tpldirectory'].'/'; //DIYÄ£°åÎÄ¼þÄ¿Â¼
			$preend = '_diy_preview';
			...
			if(isset($diytemplatename[$file]) && file_exists($diypath.$file.'.htm') && ($tplsavemod = 1) || empty($_G['forum']['styleid']) && ($file = $primaltpl ? $primaltpl : $oldfile) && isset($diytemplatename[$file]) && file_exists($diypath.$file.'.htm')) {
				...
				$curtplname = $file;
				if(isset($_GET['diy']) && $_GET['diy'] == 'yes' || isset($_GET['diy']) && $_GET['preview'] == 'yes') { //DIYÄ£Ê½»òÔ¤ÀÀÄ£Ê½ÏÂ×öÒÔÏÂÅÐ¶Ï
			...
			if($indiy && ($tplrefresh ==1 || ($tplrefresh > 1 && !($_G['timestamp'] % $tplrefresh))) && filemtime($diypath.$file.'.htm') < filemtime(BM_ROOT.$_G['style']['tpldirectory'].'/'.($primaltpl ? $primaltpl : $oldfile).'.htm')) {
...
/*
	...
	if(defined('IN_MOBILE') && !defined('TPL_DEFAULT')) {
		if(strpos($tpldir, 'plugin')) {
			if(!file_exists(BM_ROOT.$tpldir.'/'.$file.'.htm') && !file_exists(BM_ROOT.$tpldir.'/'.$file.'.php')) {
				bm_error::template_error('template_notfound', $tpldir.'/'.$file.'.htm');
			} else {
				$mobiletplfile = $tpldir.'/'.$file.'.htm';
			}
		}
		!$mobiletplfile && $mobiletplfile = $file.'.htm';
		if(strpos($tpldir, 'plugin') && (file_exists(BM_ROOT.$mobiletplfile) || file_exists(substr(BM_ROOT.$mobiletplfile, 0, -4).'.php'))) {
			$tplfile = $mobiletplfile;
		} elseif(!file_exists(BM_ROOT.TPLDIR.'/'.$mobiletplfile) && !file_exists(substr(BM_ROOT.TPLDIR.'/'.$mobiletplfile, 0, -4).'.php')) {
			$mobiletplfile = './view/default/'.$mobiletplfile;
			if(!file_exists(BM_ROOT.$mobiletplfile) && !$_G['forcemobilemessage']) {
	...
	$cachefile = './data/view/'.(defined('STYLEID') ? STYLEID.'_' : '_').$templateid.'_'.str_replace('/', '_', $file).'.tpl.php';
	if($templateid != 1 && !file_exists(BM_ROOT.$tplfile) && !file_exists(substr(BM_ROOT.$tplfile, 0, -4).'.php')
			&& !file_exists(BM_ROOT.($tplfile = $tpldir.$filebak.'.htm'))) {
		$tplfile = './view/default/'.$filebak.'.htm';
	}

	if($gettplfile) {
		return $tplfile;
	}
	checktplrefresh($tplfile, $tplfile, @filemtime(BM_ROOT.$cachefile), $templateid, $cachefile, $tpldir, $file);
	return BM_ROOT.$cachefile;
*/

	if(!file_exists(BM_ROOT.$tplfile)) {
		bm_error::template_error('template_notfound', $tplfile);
	}

	$fileid = str_replace('/', '_', $file);
	return jade(BM_ROOT.$tplfile, $fileid, true);
}
...
function getattachmentpath_by_attachment($att) {
	...
	if ($att['aid'] != 0) {
		...
		$subdir = $subdir1.'/'.$subdir2.'/';
		$path = '/data/attach/'.$att['type'].'/'.$subdir.$att['aid'];
		if (intval($att['fix']) > 0) {
			$path .= '_'.$att['fix'];
		}
		$path .= '.'.$att['ext'];
...
function checkactivate() {
	global $_G;
	if (!isactivate()) {
		showmessage('need_activate', '/account/profile/'.$_G['uid'].'/');
	}
}
...
function checkbrowser() {
	...
	$browser = dhtmlspecialchars($_SERVER['HTTP_USER_AGENT']);
	$_G['browser'] = $browser;
	
	if (strstr($browser, 'MSIE')) {
		if (preg_match('/MSIE ([0-9]+)/i', $browser, $matches)) {
...
function getunreadmsgscount() {
	global $_G;
	if ($_G['uid']) {
		return C::t('msgs')->getunread($_G['uid'], 1);		
	}
	return 0;
}
