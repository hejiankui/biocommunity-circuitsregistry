 ...
 *      SUSTC-ShenZhen-B
 *
 *      $Id: class/ei/error.php 2013-06-13 11:37:00 Zhuoteng Peng $
 */
...
class bm_error
{
	...
	public static function system_error($message, $show = true, $save = true, $halt = true) {
		...
		list($showtrace, $logtrace) = bm_error::debug_backtrace();

		if($save) {
			$messagesave = '<b>'.$message.'</b><br><b>PHP:</b>'.$logtrace;
			bm_error::write_error_log($messagesave);
		}

		if($show) {
			if(!defined('IN_MOBILE')) {
				bm_error::show_error('system', "<li>$message</li>", $showtrace, 0);
			} else {
				bm_error::mobile_show_error('system', "<li>$message</li>", $showtrace, 0);
			}
		}
	...
	public static function template_error($message, $tplname) {
		$message = lang('error', $message);
		$tplname = str_replace(BM_ROOT, '', $tplname);
		$message = $message.': '.$tplname;
		bm_error::system_error($message);
	}

	public static function debug_backtrace() {
		$skipfunc[] = 'bm_error->debug_backtrace';
		$skipfunc[] = 'bm_error->db_error';
		$skipfunc[] = 'bm_error->template_error';
		$skipfunc[] = 'bm_error->system_error';
		...
		foreach ($debug_backtrace as $k => $error) {
			$file = str_replace(BM_ROOT, '', $error['file']);
	...
	public static function db_error($message, $sql) {
		...
		bm_error::show_error('db', $msg, $showtrace, false);
		unset($msg, $phperror);
		...
		bm_error::write_error_log($errormsg);
		exit();
	...
	public static function exception_error($exception) {
		...
		foreach ($trace as $error) {
				...
				if(!empty($error['args'])) {
					...
					foreach($error['args'] as $arg) {
						...
						if(is_array($arg)) {
							...
							$fun .= (defined('BM_DEBUG') && BM_DEBUG) ? $arg : '%d';
						} elseif(is_float($arg)) {
							$fun .= (defined('BM_DEBUG') && BM_DEBUG) ? $arg : '%f';
						} else {
							$fun .= (defined('BM_DEBUG') && BM_DEBUG) ? '\''.dhtmlspecialchars(substr(self::clear($arg), 0, 10)).(strlen($arg) > 10 ? ' ...' : '').'\'' : '%s';
						}
						$mark = ', ';
					}
				}
			...
			$phpmsg[] = array(
			    'file' => str_replace(array(BM_ROOT, '\\'), array('', '/'), $error['file']),
			    'line' => $error['line'],
...
<tr>
	<td id="bodytitle" width="100%">BioMiao $title Error </td>
</tr>
EOT;
...
EOT;
	...
	public static function write_error_log($message) {
		...
		$message = bm_error::clear($message);
		$time = time();
		$file =  BM_ROOT.'./data/log/'.date("Ym").'_errorlog.php';
		$hash = md5($message);
		...
		$uri = 'Request: '.dhtmlspecialchars(bm_error::clear($_SERVER['REQUEST_URI']));
