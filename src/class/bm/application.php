 ...
 *      SUSTC-ShenZhen-B
 *
 *      $Id: class/biomiao/application.php 2013-06-13 11:04:00 Zhuoteng Peng $
 */
...
class bm_application extends bm_base {
	...
	private function _init_env() {
		...
		if(!defined('BM_CORE_FUNCTION') && !@include(BM_ROOT.'./src/func/core.php')) {
			exit('func/core.php is missing');
		}
		...
		//$_G['setting']['bbname'] = lang('core', 'bbname');
		//$_G['setting']['sitename'] = $_G['setting']['bbname'];
		...
		if (!checkbrowser()) {
			//if ($_GET['mod'] != 'unsupported') {
			//	dheader('Location: /unsupported/');
			//}
		}
	...
	private function _init_input() {
		...
		$this->var['inajax'] = isset($_GET['ajax']) && (intval($_GET['ajax']) != 0);
		$this->var['p'] = empty($_GET['p']) ? 1 : max(1, intval($_GET['p']));
	...
	private function _init_config() {
		...
		$_config = array();
		@include BM_ROOT.'./src/conf/global.php';
		if(empty($_config)) {
			if(!file_exists(BM_ROOT.'./data/install.lock')) {
		...
		if(empty($_config['debug']) || !file_exists(libfile('func/debug'))) {
	...
	private function _init_session() {
		...
		$this->session = $sessionclose ? new bm_session_close() : new bm_session();
	...
	private function _init_user() {
		if($this->init_user) {
			...
			list($bm_pw, $bm_uid) = empty($auth) || count($auth) < 2 ? array('', '') : $auth;

			if($bm_uid) {
				$user = getuserbyuid($bm_uid, 1);
			}

			if(!empty($user) && $user['password'] == $bm_pw) {
				if(isset($user['_inarchive'])) {
					C::t('common_member_archive')->move_to_master($bm_uid);
				}
		...
		if($this->init_user) {
			...
			if($this->var['group'] && isset($this->var['group']['allowvisit']) && !$this->var['group']['allowvisit']) {
				if($this->var['uid'] && !$allowvisitflag) {
					...
					dheader('location: account.php?mod=signin&referer='.rawurlencode($this->var['siteurl'].$this->var['basefilename'].($_SERVER['QUERY_STRING'] ? '?'.$_SERVER['QUERY_STRING'] : '')));
				}
			}
	...
	private function _init_setting() {
		...
		require_once BM_ROOT.'./src/inc/settings.php';
	}
