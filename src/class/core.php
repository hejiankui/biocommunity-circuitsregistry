 ...
 *      SUSTC-ShenZhen-B
 *
 *      $Id: core.php 2013-06-13 04:02:00 Zhuoteng Peng $
 */
...
define('IN_BIOMIAO', true);
define('BM_ROOT', substr(dirname(__FILE__), 0, -9));
define('BM_CORE_DEBUG', true);

include BM_ROOT.'./src/version.php';
...
class core
{
	...
	public static function creatapp() {
		if(!is_object(self::$_app)) {
			self::$_app = bm_application::instance();
		}
		return self::$_app;
	}
	...
	public static function handleException($exception) {
		bm_error::exception_error($exception);
	}


	public static function handleError($errno, $errstr, $errfile, $errline) {
		if($errno & BM_CORE_DEBUG) {
			bm_error::system_error($errstr, false, true, false);
		}
	}

	public static function handleShutdown() {
		if(($error = error_get_last()) && $error['type'] & BM_CORE_DEBUG) {
			bm_error::system_error($error['message'], false, true, false);
		}
	}
	...
	public static function autoload($class) {
		...
		try {
			...
			bm_error::exception_error($exc);
		}
	}
}

class C extends core {}
class DB extends bm_database {}
