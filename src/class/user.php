	...
	function on_login() {
		...
		if($_GET['formhash'] != $_G['formhash']) {
			...
			if(!($_G['member_loginperm'] = logincheck($_GET['username']))) {
				showmessage('login_strike', '/user/signin/');
			}
			
			$_G['uid'] = $_G['member']['uid'] = 0;
			$_G['username'] = $_G['member']['username'] = $_G['member']['password'] = '';
			if(!$_GET['username'] || $_GET['username'] != addslashes($_GET['username'])) {
				showmessage('profile_illegal', '/user/signin/');
			}
			if(!$_GET['password'] || $_GET['password'] != addslashes($_GET['password'])) {
				showmessage('profile_passwd_illegal', '/user/signin/');
			}
			...
			if($result['status'] > 0) {
				...
				$location = $_GET['redirect_url'] ? $_GET['redirect_url'] : ($result['member']['emailstatus'] == 0 ? '/user/' : '/');
				...
				if($_G['member_loginperm'] > 1) {
					showmessage($fmsg, '/user/signin/', array('loginperm' => $_G['member_loginperm'] - 1));
				} elseif($_G['member_loginperm'] == -1) {
					showmessage('login_password_invalid', '/user/signin/');
				} else {
					showmessage('login_strike', '/user/signin/');
				}
			}
