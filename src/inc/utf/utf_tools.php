...
function utf8_recode($string, $encoding)
{
	...
	if (preg_match('/iso[_ -]?8859[_ -]?(\\d+)/', $encoding, $array))
	{
		switch ($array[1])
		{
			...
			case '15':
				...
				{
					if (!file_exists(BM_ROOT . 'src/inc/utf/data/recode_basic.php'))
					{
						trigger_error('Basic reencoder file is missing', E_USER_ERROR);
					}
					include(BM_ROOT . 'src/inc/utf/data/recode_basic.php');
				}
	...
	if (preg_match('/sjis(?:[_ -]?win)?|(?:cp|ibm)[_ -]?932|shift[_ -]?jis/', $encoding))
	{
		...
		{
			if (!file_exists(BM_ROOT . 'src/inc/utf/data/recode_cjk.php'))
			{
				trigger_error('CJK reencoder file is missing', E_USER_ERROR);
			}
			include(BM_ROOT . 'src/inc/utf/data/recode_cjk.php');
		}
		return sjis($string);
	}
...
function utf8_case_fold($text, $option = 'full')
{
	...
	if (!isset($uniarray['c']))
	{
		$uniarray['c'] = include(BM_ROOT . 'src/inc/utf/data/case_fold_c.php');
	}

	// only set full if we need to
	if ($option === 'full' && !isset($uniarray['f']))
	{
		$uniarray['f'] = include(BM_ROOT . 'src/inc/utf/data/case_fold_f.php');
	}

	// only set simple if we need to
	if ($option !== 'full' && !isset($uniarray['s']))
	{
		$uniarray['s'] = include(BM_ROOT . 'src/inc/utf/data/case_fold_s.php');
	}
...
function utf8_case_fold_nfkc($text, $option = 'full')
{
	...
	if (!class_exists('utf_normalizer'))
	{
		...
		include(BM_ROOT . 'src/inc/utf/utf_normalizer.php');
	}
...
function utf8_clean_string($text)
{
	...
	if (empty($homographs))
	{
		$homographs = include(BM_ROOT . 'src/inc/utf/data/confusables.php');
	}
